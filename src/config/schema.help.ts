...
export const FIELD_HELP: Record<string, string> = {
  ...
  audit:
    "Postgres-backed audit logging for gateway, tool, and memory events. Use this to centralize operational auditing, incident investigation, and memory change tracking.",
  "audit.enabled":
    "Enables audit event capture and Postgres writes. Keep disabled when no audit sink is configured.",
  "audit.teamId":
    "Logical team identifier stored with each audit event for multi-tenant or fleet aggregation.",
  "audit.instanceId":
    "Logical instance identifier stored with each audit event to distinguish runtime sources.",
  "audit.pg":
    "Postgres connection and storage settings for audit events and memory patches.",
  "audit.pg.connectionString":
    "Postgres connection string for the audit sink. This is sensitive and should be protected like credentials.",
  "audit.pg.schema":
    "Schema name used for audit tables. Defaults to the Postgres default schema when omitted.",
  "audit.pg.eventsTable":
    "Table name for audit event rows. Use a dedicated table for clear retention and indexing policy.",
  "audit.pg.memoryTable":
    "Table name for memory diff patch rows. Use a separate table if you want different retention/indexing.",
  "audit.pg.maxBatchSize":
    "Maximum number of audit rows buffered per insert batch. Larger batches improve throughput at the cost of latency.",
  "audit.pg.flushIntervalMs":
    "Flush interval in milliseconds for draining queued audit rows to Postgres.",
  "audit.agentEvents":
    "Agent event stream capture settings for audit logging.",
  "audit.agentEvents.enabled":
    "Enable agent event stream ingestion into audit storage.",
  "audit.agentEvents.streams":
    "Agent event streams to include in audit logging (for example: tool, assistant).",
  "audit.memory":
    "Memory file change capture settings for audit logging.",
  "audit.memory.enabled":
    "Enable memory file diff capture into the audit sink.",
  "audit.memory.includeGlobs":
    "Glob list of memory files to watch for diffs. Defaults to MEMORY.md and memory/**/*.md within the workspace.",
  "audit.memory.ignoreGlobs":
    "Glob list of paths to ignore for memory diff capture (for example: node_modules, .git).",
